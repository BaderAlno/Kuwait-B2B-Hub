  ...
  useEffect(() => {
    fetch(`/api/products/${id}`)
      ...
      .then(data => {
        ...
        if (!p) { setError('Product not found'); setFetching(false); return; }
        setForm({
          name: p.name || '',
        ...
        setTiers(
          (p.bulk_pricing_tiers || []).map((t: { min_qty: number; max_qty: number | null; price: number }) => ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`/api/products/${id}`, {
        ...
        body: JSON.stringify({ ...form, price: parseFloat(form.price), moq: parseInt(form.moq), stock: parseInt(form.stock), bulk_pricing_tiers }),
      });
      ...
      <div className="dash-layout">
        ...
        <main className="dash-main fade-up">
          <div className="page-header">
            <div>
              <h1>Edit Product</h1>
              <p>Update your wholesale listing and pricing tiers</p>
            </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
                  ...
                  <button type="submit" className="btn btn-primary" style={{ flex: 1 }}
                    ...
                    {loading ? <><span className="spinner"/> Saving…</> : <><ShoppingBag size={15}/> Save Changes</>}
                  </button>
                </div>
              </div>
            </div>
